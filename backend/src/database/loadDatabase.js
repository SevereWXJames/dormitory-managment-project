...
export default function loadSampleData() {
    ...
    return database.database.dropDatabase().then((value) => Promise.all([database.load("credit_balances", creditBalanceJSON.creditBalances),
        ...
        database.load("services", serviceJSON.services)])).then((value) => {
        Promise.resolve();
    })
        .catch((e) => {
        Promise.reject(e);
